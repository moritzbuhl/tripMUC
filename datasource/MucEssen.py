...
for page in pages:
    ...
    response = get('https://www.muenchen.de/essen-trinken/gaststaetten-restaurants_' + page + '.html', headers = basicHeaders)
    html_soup = BeautifulSoup(response.text, 'lxml')
    if html_soup is not None:
        eat_containers = html_soup.find_all('div', class_= ['item item--premium','item item--business','item item--grund'])
...
test_df.to_csv('mucessen.csv')
test_df.to_json('mucessen.json')
print(test_df.info())